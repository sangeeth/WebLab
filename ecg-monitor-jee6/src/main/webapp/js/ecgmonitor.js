function ECGMonitor(canvasId) {
	...
	// 8 pixels/100ms = 25 mm/sec
	// 16 pixels/100ms = 50 mm/sec
	// where 1 mm = 3pixels
	...
	this.startup = function() {
		...
		this.canvasContext.fillRect(0, 0, this.canvasContext.canvas.width,
		...
		this.canvasContext.fillText("II", 10, this.startY + this.canvasHeight
				/ 2 - 20);
	...
	this.paint = function() {
		...
		this.canvasContext.fillRect(this.lastX, this.startY,
				this.pixelsPer100ms, this.canvasHeight);
		var diff = this.lastX + this.pixelsPer100ms - this.canvasWidth;
		if (diff >= 0) {
			this.canvasContext.fillRect(this.startX, this.startY, diff,
					this.canvasHeight);
		}
		...
		this.canvasContext.beginPath();
		{
			...
			while (count > 0) {
				...
				data = this.next();
				newY = this.y_axis_y
						- ((data - this.lowerLimit) * this.y_axis_scale);
